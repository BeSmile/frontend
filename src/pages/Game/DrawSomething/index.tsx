...
export default () => {
  ...
  // 画线
  const handleLineMove = (downX: number, downY: number, type: string) => {
    const { current: wrapper } = wrapperRef;
    const { current: canvas } = canvasRef;
    ...
    const { pointX, pointY } = generateLinePoint(
      downX,
      downY
    );
    ...
    // 设置画笔起点
    context.moveTo(
      getPixelPoint(pointX),
      getPixelPoint(pointY)
    );
          ...
          switch (data.action) {
            case "start":
              context.globalCompositeOperation =
                msgObject.type === LINE_MODE ? "source-over" : "";
              context.beginPath();
              // 设置画笔起点
              context.moveTo(
                getPixelPoint(point.x),
                getPixelPoint(point.y)
              );
              break;
            case "move":
              // 设置画笔起点
  ...
  useEffect(() => {
    const draw$: any = fromEvent(
      drawRef.current,
      "click"
    );
    const eraser$: any = fromEvent(
      eraserRef.current,
      "click"
    );
