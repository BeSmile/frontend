...
const DesignPropertiesFC = memo((props: DesignPropertiesProps) => {
  ...
  const renderSchemaJSX = (schema): React.ReactElement => {
    ...
    Object.keys(schema.properties).forEach((proto: string) => {
      tabElements.push(
        <div
          key={`tab-${proto}`}
          className={styles.designPropertiesContainerTabsItem}
        >
          {proto}
        </div>
      );
    ...
    return (
      <>
        ...
        <div className={styles.designPropertiesContainer}>
          <div className={styles.designPropertiesContainerTabs}>
            {tabElements}
          </div>
          {elements}
        </div>
  ...
  return (
    <div className={styles.designProperties}>{renderSchemaJSX(schema)}</div>
  );
});
